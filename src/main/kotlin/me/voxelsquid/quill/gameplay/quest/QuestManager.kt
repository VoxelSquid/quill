package me.voxelsquid.quill.gameplay.quest
...
import me.voxelsquid.quill.gameplay.humanoid.HumanoidManager.HumanoidEntityExtension.getHumanoidController
import me.voxelsquid.quill.gameplay.humanoid.HumanoidManager.HumanoidEntityExtension.hunger
...
class QuestManager(private val plugin: QuestIntelligence) {
    ...
    fun prepareQuest() {
        ...
        if (plugin.allowedWorlds.isEmpty()) {
        ...
        if (villager.persistentDataContainer.get(personalDataKey, PersistentDataType.STRING) == null) {
            plugin.debug("Villager at ${villager.location}.")
    ...
    private fun getRandomVillager(): Villager? {
        val villagers = plugin.allowedWorlds.random().entities.filterIsInstance<Villager>()
