...
class VersionBridge(private val plugin: JavaPlugin) : AbstractVersionBridge() {
    ...
    override val attributes: Map<UniversalAttribute, Attribute>
        get() = mutableMapOf<UniversalAttribute, Attribute>().apply {
            put(UniversalAttribute.MAX_HEALTH, Attribute.MAX_HEALTH)
            put(UniversalAttribute.ATTACK_SPEED, Attribute.ATTACK_SPEED)
            put(UniversalAttribute.ATTACK_DAMAGE, Attribute.ATTACK_DAMAGE)
            put(UniversalAttribute.PROTECTION, Attribute.ARMOR)
            put(UniversalAttribute.ARMOR_TOUGHNESS, Attribute.ARMOR_TOUGHNESS)
            put(UniversalAttribute.SCALE, Attribute.SCALE)
            put(UniversalAttribute.BLOCK_BREAK_SPEED, Attribute.BLOCK_BREAK_SPEED)
            put(UniversalAttribute.BLOCK_INTERACTION_RANGE, Attribute.BLOCK_INTERACTION_RANGE)
            put(UniversalAttribute.ENTITY_INTERACTION_RANGE, Attribute.ENTITY_INTERACTION_RANGE)
        }
    ...
    override fun getOminousBanner() : ItemStack {
        return CraftItemStack.asBukkitCopy(
            Raid.getOminousBannerInstance((plugin.server.worlds.random() as CraftWorld).handle.registryAccess().lookupOrThrow(
    ...
    override fun addAttributeModifier(
        ...
        attributes[attribute]?.let { meta.addAttributeModifier(it, AttributeModifier(NamespacedKey(plugin, "${attribute}_${slot}"), amount, operation, slot)) }
    }
